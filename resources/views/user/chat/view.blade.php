        ...
        <div class="page-leftheader">
            ...
            <ol class="breadcrumb mb-2">
                <li class="breadcrumb-item"><a href="{{ route('user.dashboard') }}"><i
                            class="fa-solid fa-messages-question mr-2 fs-12"></i>{{ __('User') }}</a></li>
                <li class="breadcrumb-item" aria-current="page"><a href="{{ route('user.chat') }}">
                        {{ __('AI Chat Assistants') }}</a></li>
        ...
        <div class="page-rightheader">
            <div id="balance-status">
                <span class="fs-11 text-muted pl-3"><i
                        class="fa-sharp fa-solid fa-bolt-lightning mr-2 text-primary"></i>{{ __('Your Balance is') }} <span
                ...
                <div class="chat-sidebar-container">
                    ...
                    <div class="chat-sidebar-messages">
                        @foreach ($messages as $key => $message)
                            <div class="chat-sidebar-message @if ($loop->first) selected-message @endif"
                                id="{{ $message->conversation_id }}">
                                <h6 class="chat-title" id="title-{{ $message->conversation_id }}">
                                    {{ __($message->title) }}
                                </h6>
                                <div class="chat-info">
                                    <div class="chat-count"><span>{{ $message->messages }}</span> {{ __('messages') }}
                                    </div>
                                    <div class="chat-date">
                                        {{ \Carbon\Carbon::parse($message->updated_at)->diffForhumans() }}</div>
                                </div>
                                <div class="chat-actions d-flex">
                                    <a href="#" class="chat-edit fs-12" id="{{ $message->conversation_id }}"><i
                                            class="fa-sharp fa-solid fa-pen-to-square"
                                            data-tippy-content="{{ __('Edit Name') }}"></i></a>
                                    <a href="#" class="chat-delete fs-12 ml-2"
                                        id="{{ $message->conversation_id }}"><i class="fa-sharp fa-solid fa-trash"
                                            data-tippy-content="{{ __('Delete Chat') }}"></i></a>
                                </div>
                            </div>
                        @endforeach
                    </div>
                        ...
                        <div class="row text-center">
                            <div class="col-sm-12">
                                <a class="btn btn-primary pl-6 pr-6 fs-12"
                                    id="new-chat-button">{{ __('New Conversation') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-header">
                        <div class="w-100 pt-2 pb-2">
                            <div class="d-flex">
                                <div class="overflow-hidden mr-4"><img alt="Avatar" class="chat-avatar"
                                        src="{{ URL::asset($chat->logo) }}"></div>
                                <div class="widget-user-name"><span
                                        class="font-weight-bold">{{ __($chat->name) }}</span><br><span
                                        class="text-muted">{{ __($chat->sub_name) }}</span></div>
                            </div>
                        </div>
                        <div class="w-50 text-right pt-2 pb-2">
                            <a id="expand" class="template-button" href="#"><i
                                    ...
                                    data-tippy-content="{{ __('Show Chat Conversations') }}"></i></a>
                            <a id="export-word" class="template-button mr-2" onclick="exportWord();" href="#"><i
                                    ...
                                    data-tippy-content="{{ __('Export Chat Conversation as Word File') }}"></i></a>
                            <a id="export-pdf" class="template-button mr-2" onclick="exportPDF();" href="#"><i
                                    ...
                                    data-tippy-content="{{ __('Export Chat Conversation as PDF File') }}"></i></a>
                            <a id="export-txt" class="template-button mr-2" onclick="exportTXT();" href="#"><i
                                    ...
                                    data-tippy-content="{{ __('Export Chat Conversation Text File') }}"></i></a>
                            ...
                            <div class="col-md-12 col-sm-12">
                                <div id="chat-container">
                                    <div class="msg left-msg">
                                        <div class="message-img" style="background-image: url({{ $chat->logo }})">
                                        </div>
                                    ...
                                    <div class="chat-controllers">
                                        @if (config('settings.vision_for_chat_feature_user') == 'allow')
                                            ...
                                            <div class="upload-button-nonresponsive"><a class="btn chat-button-icon"
                                                    ...
                                                    data-tippy-content="{{ __('Select your Image') }}"><i
                                                        class="fa-solid fa-image"></i></a></div>
                                        @endif
                                        <textarea type="message" class="form-control @error('message') is-danger @enderror" rows="1" id="message"
                                            name="message" placeholder="{{ __('Type your message here...') }}"></textarea>
                                        @if (config('settings.vision_for_chat_feature_user') == 'allow')
                                        ...
                                        <div class="chat-button-box"><a class="btn chat-button-icon"
                                                ...
                                                data-tippy-content="{{ __('Prompt Library') }}"><i
                                        ...
                                        <div class="">
                                            <label class="custom-switch">
                                                ...
                                                <span class="custom-switch-indicator"></span>
                                                <span
                                                    class="custom-switch-description col-md-12 m-0 p-0">{{ __('Open AI') }}</span>
                                            </label>
                                        </div>
                                        <div><button class="btn chat-button" id="chat-button">{{ __('Send') }} <i
                                    ...
                                    <div class="input-box">
                                        <div class="form-group prompt-search-bar-dark">
                                            <input type="text" class="form-control" id="search-template"
                                                placeholder="{{ __('Search for prompts...') }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="col-md-6 col-sm-12" id="{{ $prompt->group }}">
                                            <div class="prompt-boxes">
                                                <div class="card border-0"
                                                    onclick='applyPrompt("{{ __($prompt->prompt) }}")'>
                                                    <div class="card-body pt-3">
                                                        <div class="template-title">
                                                            <h6 class="mb-2 fs-15 number-font">{{ __($prompt->title) }}
                                                            </h6>
                                                        </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data == 'success') {
                            ...
                            toastr.warning(
                                '{{ __('There was an issue while deleting chat conversation') }}');
                        }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data == 'success') {
                        ...
                        toastr.warning(
                            '{{ __('There was an issue while creating chat conversation') }}');
                    }
        ...
        $(document).on('click', '.chat-edit', function(e) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data['status'] == 'success') {
                                toastr.success(
                                    '{{ __('Chat title has been updated successfully') }}');
                                document.getElementById("title-" + data['conversation_id'])
                                    .innerHTML = result.value;
                            } else {
                                toastr.error(
                                    '{{ __('Chat title was not updated correctly') }}');
                            }
                        },
                        error: function(data) {
                    ...
                    Swal.fire('{{ __('No Title Entered') }}',
                        '{{ __('Make sure to provide a new chat title before updating') }}', 'warning')
                }
            })
        });
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data['status'] == 'success') {
                                toastr.success(
                                    '{{ __('Chat history has been successfully deleted') }}'
                                    );
    ...
    function appendMessageSpecial(img, side, text, code, code) {
        ...
        function escape_html(str) {
            ...
            str = str.replaceAll('</code></pre>',
                '</code><button type="button" class="copy-code" onclick="copyCode(this)"><span class="label-copy-code">{{ __('Copy') }}</span></button></pre>'
                );
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data['status'] == 'success') {
                        ...
                        toastr.error(
                            '{{ __('Audio player is not available, please try again or contact support') }}'
                            );
                    }
                },
                error: function(data) {
