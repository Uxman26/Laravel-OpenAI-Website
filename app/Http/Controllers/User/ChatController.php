...
class ChatController extends Controller
{
    ...
    public function process(Request $request)
    {
        ...
        session()->put('open_ai_chat', $request->open_ai_chat);
    ...
    public function generateChat(Request $request)
    {
        ...
        $open_ai_chat = session()->get('open_ai_chat');
        $chat_id_llm = session()->get('chat_id_llm');
        ...
        return response()->stream(function () use ($conversation_id, $googlePrompt, $open_ai_chat, $chat_id_llm) {
            ...
            $text = "";
            if ($open_ai_chat == 'off' && $chat_id_llm == null) {
                $main_chat = Chat::where('chat_code', $chat_conversation->chat_code)->first();
                // $payload = [
                //     "key" => "jZj9THVB9llqEEAiYXjDKUzRS6tRTAa2akrveVhkLcNIOVFIIzGZs19zxSXb",
                //     "model_id"  => "zephyr-7b-beta",
                //     "system_prompt" => $main_chat->prompt,
                ...
                // session()->put('chat_id_llm', $response->chat_id);
                ...
                $headers = array();
                $headers[] = 'Authorization: Token r8_e5Y12Jr3VSmFn85L6Td9oHDYoNMpJv42GFV7r';
                $headers[] = 'Content-Type: application/json';
                curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);

                $response = curl_exec($ch);
                $response = json_decode($response);
                $getApi = $response->urls->get;
                // usleep(10000000);
                for ($i = 0; $i < 100; $i++) {
                    $ch = curl_init();
                    ...
                    if ($status == "succeeded" || $i == 99) {
                    $longtext = $response->output;
                    foreach ($longtext as $data) {
                        if ($data != '' && $data != null) {
                            $raw = $data;
                            $clean = str_replace(["\r\n", "\r", "\n"], "", $raw);
                            $text .= $clean;
            ...
            } else if (is_null($chat_message->images)) {
